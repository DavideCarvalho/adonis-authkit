...
`@dudousxd/adonis-authkit-react` ships a small set of accessible, themeable components built
on top of the [headless hooks](/docs/react#6-headless-hooks). They consume the same auth
state and `AuthkitProvider` config — no extra wiring beyond what
