...
| `rateLimit` | `RateLimitConfigInput` | Opt-in; throttles login/signup/forgot/reset + PAT introspection. Only `enabled` and `store` belong here (infra); bucket values are in the `rate_limit` runtime setting. |
