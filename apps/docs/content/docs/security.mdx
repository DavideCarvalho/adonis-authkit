...
<Callout type="info">
  **Client side:** the AuthKit client resolvers accept these tokens regardless — the `cnf`
  binding travels in the introspection result. Generating DPoP proofs in the client
  (per-request signed headers) is **not implemented** and is documented as future work; for
  now, enable DPoP on the server and validate via introspection.
</Callout>
