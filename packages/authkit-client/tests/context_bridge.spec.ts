...
test.group("observability/context_bridge", () => {
  ...
  test("populateContext é no-op (não lança) quando o slot está ausente", ({
    assert,
  }) => {
  ...
  test("populateContext não propaga erro do slot (best-effort)", ({
    assert,
  }) => {
    const prev = (globalThis as Record<symbol, unknown>)[SET_SLOT];
...
test.group(
  "observability/context_bridge — via Authenticator.getIdentity",
  () => {
    test("getIdentity popula o contexto no caminho de sucesso", async ({
      assert,
    }) => {
      const { patches, restore } = installFakeSet();
      try {
    ...
    test("getIdentity NÃO escreve no contexto para sessão anônima (null)", async ({
      assert,
    }) => {
      const { patches, restore } = installFakeSet();
      try {
    ...
    test("getIdentity não lança quando o slot de contexto está ausente", async ({
      assert,
    }) => {
      const restore = clearSlot();
      try {
