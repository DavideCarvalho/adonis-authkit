...
import { importJWK, jwtVerify, decodeProtectedHeader, base64url } from 'jose'
import { createHash } from 'node:crypto'
import { generateDpopKeyPair, createDpopProof, dpopJwkThumbprint } from '../src/dpop.js'
...
test.group('DPoP (client)', () => {
  ...
  test('createDpopProof produz JWT que verifica com a JWK pública embutida', async ({
    assert,
  }) => {
    const kp = await generateDpopKeyPair()
