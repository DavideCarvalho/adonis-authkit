...
test.group('AuthkitClientManager', (group) => {
  ...
  test('createAuthenticator resolve identity da session', async ({ assert }) => {
    const resolved = await configProvider.resolve({} as any, defineConfig({
      issuer: ISSUER, clientId: 'app1', redirectUri: `${ISSUER}/cb`, resolver: resolvers.jwt({ tokenSource: 'session' }),
