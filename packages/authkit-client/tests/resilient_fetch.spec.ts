import { retry, wrap } from '@adonis-agora/resilience';
import { test } from '@japa/runner';
import { __clearDiscoveryCacheForTests, discoverEndpoints } from '../src/discovery.js';
import { type ResiliencePolicy, resilientFetch } from '../src/http/resilient_fetch.js';
...
test.group('resilientFetch helper', () => {
  test('sem política → passthrough puro (uma única chamada de fetch)', async ({ assert }) => {
    const f = countingFetch({ ok: true });
    const res = await resilientFetch('https://idp.test/x', undefined, undefined, f.impl);
    assert.equal(f.calls(), 1);
    assert.isTrue(res.ok);
  });

  test('com política → roda fetch dentro de policy.execute', async ({ assert }) => {
    const f = countingFetch({ ok: true });
    let wrapped = 0;
  ...
  test('política que retenta → o fetch é re-executado', async ({ assert }) => {
    ...
    const res = await resilientFetch('https://idp.test/x', undefined, retrying, flaky);
    assert.equal(calls, 3);
    assert.isTrue(res.ok);
  });

  test('wrap/retry reais de @adonis-agora/resilience envolvem a chamada', async ({ assert }) => {
    let calls = 0;
    const flaky = (async () => {
      calls++;
    ...
    const res = await resilientFetch('https://idp.test/x', undefined, policy, flaky);
...
test.group('resilience threaded através de discovery + oidc_login', (group) => {
  ...
  test('discoverEndpoints sem resilience → fetch chamado uma vez', async ({ assert }) => {
    let calls = 0;
    const f = (async () => {
      calls++;
  ...
  test('discoverEndpoints com política retentadora → re-executa o fetch', async ({ assert }) => {
    let calls = 0;
    const f = (async () => {
      calls++;
  ...
  test('exchangeCode sem resilience → uma única chamada (back-compat)', async ({ assert }) => {
    let calls = 0;
    const f = (async () => {
      calls++;
