...
test.group("resolveConfig", () => {
  ...
  test("canPath sobrescreve o default de endpoints.can", ({ assert }) => {
    assert.equal(
      resolveConfig({ canPath: "/api/authz/can" }).endpoints.can,
      "/api/authz/can",
    );
  });

  test("endpoints.can vence canPath quando ambos são passados", ({
    assert,
  }) => {
...
test.group("buildAuthUrl", () => {
  ...
  test("usa & quando a base já tem query", ({ assert }) => {
    assert.equal(
      buildAuthUrl("/auth/login?x=1", "/a"),
      "/auth/login?x=1&returnTo=%2Fa",
    );
  });
});
