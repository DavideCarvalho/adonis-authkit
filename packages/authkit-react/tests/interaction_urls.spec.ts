...
} from "../src/components/interaction_form.js";
...
test.group("interactionUrls (tier 3)", () => {
  ...
  test("the uid is interpolated into the path", ({ assert }) => {
    assert.equal(
      interactionUrls("uid-1").login,
      "/auth/interaction/uid-1/login",
    );
    assert.equal(
      interactionUrls("uid-2").login,
      "/auth/interaction/uid-2/login",
    );
  });
});
...
test.group("oauthRedirectUrl (tier 3)", () => {
  test("builds the provider redirect URL", ({ assert }) => {
    assert.equal(
      oauthRedirectUrl("google", "abc"),
      "/auth/google/redirect/abc",
    );
    assert.equal(
      oauthRedirectUrl("github", "abc"),
      "/auth/github/redirect/abc",
    );
  });

  test("honors a custom base path", ({ assert }) => {
    assert.equal(
      oauthRedirectUrl("google", "abc", "/sso"),
      "/sso/google/redirect/abc",
    );
  });
});
...
test.group("buttonClass helper", () => {
  test("composes base + variant + extra, dropping empties", ({ assert }) => {
    ...
    assert.equal(
      buttonClass("authkit-button--primary"),
      "authkit-button authkit-button--primary",
    );
...
test.group(
  "InteractionForm / MagicLinkButton / OAuthButton — exports e types",
  () => {
    ...
    test("InteractionFormProps.step is a POST interaction step", ({
      assert,
    }) => {
