...
export default class AuthkitExpireScan extends BaseCommand {
  ...
  @flags.boolean({ description: 'Envia e-mail de aviso às contas a expirar em breve (deduplicado via audit).' })
  ...
  async run() {
    ...
    this.logger.info(`[expire-scan] Expiring soon (within ${summary.warnDays}d): ${summary.warnSoon}`)
    if (result.warned > 0) {
