...
function toDateStr(val: any): string {
  ...
  if (typeof val === 'object' && 'toISO' in val) return (val as any).toISO() ?? new Date().toISOString()
  ...
  return {
    ...
    async acceptInvitation(invitationId, accountId) {
      ...
      if (rawExpiry && typeof rawExpiry === 'object' && 'toMillis' in rawExpiry) {
        ...
        const normalized = String(rawExpiry).replace(' ', 'T') + (String(rawExpiry).includes('Z') ? '' : 'Z')
        expiresMs = new Date(normalized).getTime()
      } else {
        expiresMs = 0
      }
      if (isNaN(expiresMs) || expiresMs < Date.now()) return { ok: false, reason: 'expired' as const }
      // Verifica e-mail
