...
export interface AccountExport {
  ...
  linkedIdentities: Array<{ provider: string; providerUserId: string; email: string | null }>
...
export class AccountExportService {
  ...
  /** Gera o export, ou null se a conta não existe. */
  async export(accountId: string): Promise<AccountExport | null> {
    ...
    if (cfg.audit && typeof cfg.audit.list === 'function') {
      try {
        const page = await cfg.audit.list({ subject: accountId, page: 1, limit: AUDIT_EXPORT_LIMIT })
