...
export default class ConsoleUsersController {
  ...
  async index(ctx: HttpContext) {
    ...
    const perPage = Math.max(
      ...
      Math.min(
        100,
        Number.parseInt(ctx.request.input("perPage", String(PAGE_SIZE)), 10) ||
          PAGE_SIZE,
      ),
    );
  ...
  async updateRoles(ctx: HttpContext) {
    ...
    const { roles: rolesInput } = await ctx.request.validateUsing(
      adminUserRolesValidator,
    );
    const roles = Array.from(
      new Set((rolesInput ?? []).map((r) => r.trim()).filter(Boolean)),
    );
    ...
    const runtimeSettings: RuntimeSettings | null =
      ...
      return ctx.response
        ...
        .send(
          apiError(
            ...
            "O store não suporta habilitar/desabilitar contas.",
          ),
        );
    }
    const updated = await cfg.accountStore.findById(id);
  ...
  async destroy(ctx: HttpContext) {
    ...
    const enqueue = cfg.accountLifecycle?.durable
      ...
      : undefined;
    const outcome = await users.delete(
      service,
      id,
      { actorId, ip, source: "admin" },
      enqueue,
    ...
    if (!outcome.ok) {
      ...
      return ctx.response
        ...
        .send(
          apiError(
            "capability_unsupported",
            "O store não suporta deleção de contas.",
          ),
        );
    }
