...
function memoryAccountStore(): AccountStore {
  ...
  const findByEmail = (email: string) =>
    [...byId.values()].find((a) => a.email === email) ?? null
  return {
...
test.group('Admin REST API (controllers)', (group) => {
  ...
  test('users CRUD roundtrip: create → get → patch roles → disable → login blocked → enable', async ({
    ...
    const before = await attemptPasswordLogin(cfg, { email: 'jane@x.com', password: 'pw-secret-123', ip: null })
    ...
    // login blocked
    const after = await attemptPasswordLogin(cfg, { email: 'jane@x.com', password: 'pw-secret-123', ip: null })
  ...
  test('create com email duplicado → 409', async ({ assert }) => {
    const users = new ApiUsersController()
    await users.store(fakeCtx({ service, inputs: { email: 'dup@x.com', password: 'pw-secret-123' } }).ctx)
  ...
  test('sessions list + revoke-sessions remove os grants', async ({ assert }) => {
    ...
    await new DatabaseAdapter('Session', db).upsert(`sess-${id}`, { accountId: id, loginTs: 1 } as any, 3600)
    await new DatabaseAdapter('Grant', db).upsert(`grant-${id}`, { accountId: id, clientId: 'c1' } as any, 3600)
  ...
  test('audit list filtra por type', async ({ assert }) => {
    const users = new ApiUsersController()
    await users.store(fakeCtx({ service, inputs: { email: 'a1@x.com', password: 'pw-secret-123' } }).ctx)
    await users.store(fakeCtx({ service, inputs: { email: 'a2@x.com', password: 'pw-secret-123' } }).ctx)

    const misc = new ApiMiscController()
    const all: any = await misc.audit(fakeCtx({ service, inputs: {} }).ctx)
    assert.isAtLeast(all.total, 2)
    const filtered: any = await misc.audit(fakeCtx({ service, inputs: { type: 'user.created' } }).ctx)
