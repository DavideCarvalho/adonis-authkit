...
function fakeApiCtx(opts: {
  ...
  const ctx = {
    ...
    response: {
      notFound: (data: any) => {
        captured._status = 404;
        return data;
      },
      badRequest: (data: any) => {
        captured._status = 400;
        return data;
      },
    },
    containerResolver: {
...
function buildService(
  withBotProtection = true,
  opts: { social?: string[]; magicLink?: boolean; passkeyCapable?: boolean } = {},
) {
  ...
  const config = {
    botProtection: withBotProtection
      ? { verify: async () => true, on: ['login', 'signup'] }
      : undefined,
...
test.group('Admin REST API — /settings', (group) => {
  ...
  test('PUT /settings/:key creates a setting', async ({ assert }) => {
    ...
    const { ctx } = fakeApiCtx({
      service,
      db: settingsDb,
      params: { key: 'bot_protection' },
      body: { value },
    });
  ...
  test('GET /settings/:key returns saved value', async ({ assert }) => {
    await settingsDb
      .table('auth_settings')
      .insert({ key: 'test_key', value: '"hello"', updated_at: new Date(), updated_by: null });
    const ctrl = new ApiSettingsController();
  ...
  test('DELETE /settings/:key removes setting', async ({ assert }) => {
    await settingsDb
      .table('auth_settings')
      .insert({ key: 'to_delete', value: '"x"', updated_at: new Date(), updated_by: null });
    const ctrl = new ApiSettingsController();
