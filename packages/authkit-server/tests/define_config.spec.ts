...
test.group("defineConfig (server)", () => {
  test("resolve config com adapter, jwks, ttl e findAccount materializados", async ({
    assert,
  }) => {
    const fakeApp = {
  ...
  test("resolve config com host-kit (render/branding/social/mountPath/patIntrospectionSecret)", async ({
    ...
    const RedisMock = (await import("ioredis-mock")).default;
    const { configProvider } = await import("@adonisjs/core");
    const { inertiaRenderer } =
      await import("../src/host/renderers/inertia_renderer.js");
  ...
  test("rateLimit resolve defaults quando enabled sem buckets explícitos", async ({
    assert,
  }) => {
    const RedisMock = (await import("ioredis-mock")).default;
    const { configProvider } = await import("@adonisjs/core");
  ...
  test("audit é sempre o fan-out de diagnostics (sempre ligado) mesmo sem sink", async ({
    ...
    await assert.doesNotReject(() =>
      resolved.audit!.record({ type: "login.success" }),
    );
  });
  ...
  test("resolveDynamicRegistration: management sem enabled lança erro de config", async ({
    ...
    const { resolveDynamicRegistration } =
      await import("../src/define_config.js");
    assert.throws(
  ...
  test("resolveDynamicRegistration: defaults desligados", async ({
    assert,
  }) => {
    const { resolveDynamicRegistration } =
      await import("../src/define_config.js");
  ...
  test("defineConfig lança em resolve quando management sem enabled", async ({
    assert,
  }) => {
    const RedisMock = (await import("ioredis-mock")).default;
    const { configProvider } = await import("@adonisjs/core");
  ...
  test("accessTokens jwt é materializado na config resolvida", async ({
    assert,
  }) => {
    const RedisMock = (await import("ioredis-mock")).default;
    const { configProvider } = await import("@adonisjs/core");
    ...
    assert.equal(
      resolved.accessTokens.resources["https://api.test"].format,
      "jwt",
    );
  });
});
...
test.group("resolveAccessTokens", () => {
  ...
  test("format jwt (modo simples) → anyJwt true, audience default = issuer", async ({
    assert,
  }) => {
    const { resolveAccessTokens } = await import("../src/define_config.js");
  ...
  test("resources herdam format raiz; audience default = a chave (resource indicator)", async ({
    ...
    assert.equal(
      r.resources["https://api.acme.test"].audience,
      "https://api.acme.test",
    ); // = a chave
