  ...
  test('serve /.well-known/openid-configuration', async ({ assert }) => {
    ...
    const cfg = await configProvider.resolve(
      ...
      defineConfig({
        ...
        jwks: { source: 'managed' },
        clients: [{ clientId: 'app1', clientSecret: 's', redirectUris: ['https://app1/cb'], grants: ['authorization_code', 'refresh_token'] }],
  ...
  test('montado sob /oidc anuncia endpoints com o prefixo correto', async ({ assert }) => {
    ...
    const cfg = await configProvider.resolve(
      ...
      defineConfig({
        ...
        jwks: { source: 'managed' },
        clients: [
          { clientId: 'app1', clientSecret: 's', redirectUris: ['https://app1/cb'], grants: ['authorization_code'] },
        ],
        accountStore: fakeAccountStore(),
      })
    )
