...
import { defineConfig, adapters, type AuthServerConfigInput } from '../../src/define_config.js'
...
async function startServer(
  ...
  const cfg = await configProvider.resolve(
    ...
    defineConfig({
      ...
      clients: [{ clientId: CLIENT_ID, clientSecret: CLIENT_SECRET, redirectUris: [REDIRECT_URI], grants }],
...
async function exchangeCode(code: string, verifier: string, extra: Record<string, string> = {}): Promise<any> {
...
async function loginAndExchange(extraAuthorize: Record<string, string> = {}): Promise<any> {
  ...
  const login = await postForm(jar, `${ISSUER}/auth/interaction/${uid}/login`, { password: PASSWORD })
