...
function memoryAccountStore(
  ...
  const store: AccountStore & { accounts: typeof accounts } = {
    ...
    create: async (input) => {
      const id = randomUUID()
      const acc: any = { id, email: input.email, name: input.fullName ?? undefined, globalRoles: input.globalRoles ?? [], password: input.password }
      accounts.set(id, acc)
      return acc
    },
...
test.group('AccountApiController — PATCH /account/api/profile', (group) => {
  ...
  test('store sem updateProfile → 422', async ({ assert }) => {
    const store = memoryAccountStore()
    delete (store as any).updateProfile
    const started = await makeService(globalPort++, db, store)
    const ctrl = new AccountApiController()
...
test.group('AccountApiController — DELETE /account/api/sessions/:id', (group) => {
  ...
  test('sessão inexistente → 404 ou 422 (sem list no adapter)', async ({ assert }) => {
    ...
    assert.isTrue([404, 422].includes(captured.status()), `Expected 404 or 422, got ${captured.status()}`)
  })
})
...
test.group('AccountApiController — GET /account/api/apps', (group) => {
  ...
  test('DELETE revokeApp retorna resultado', async ({ assert }) => {
    ...
    assert.isTrue([200, 422].includes(captured.status()), `Expected 200 or 422, got ${captured.status()}`)
...
test.group('AccountApiController — GET /account/api/passkeys', (group) => {
  ...
  test('store com passkeys → supported:true + array', async ({ assert }) => {
    const storeWithPk = memoryAccountStore({
      listPasskeys: async () => [{ id: 'pk1', label: 'My Yubikey', createdAt: '2026-01-01T00:00:00.000Z' }],
    } as any)
...
test.group('AccountApiController — Orgs', (group) => {
  ...
  const orgStore: AccountStore = {
    ...
    findOrgById: async (id) => (id === 'org1' ? { id: 'org1', name: 'Acme', slug: 'acme', logoUrl: null } : null),
...
test.group('registerAuthHost — /account/api/* rotas registradas', () => {
  function fakeRouter() {
    ...
    return {
      ...
      put: mk('PUT'),
      any: mk('ANY'),
      group: (cb: () => void) => { cb(); return groupChain },
      routes,
    } as any
  }
  ...
  test('GET /account/api/me registrada', ({ assert }) => {
    ...
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/me' && r.method === 'GET'))
  })

  test('GET /account/api/security registrada', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/security' && r.method === 'GET'))
  })

  test('PATCH /account/api/profile registrada', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/profile' && r.method === 'PATCH'))
  })

  test('POST /account/api/password registrada', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/password' && r.method === 'POST'))
  })

  test('POST /account/api/email-change + cancel registradas', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/email-change' && r.method === 'POST'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/email-change/cancel' && r.method === 'POST'))
  })

  test('GET/DELETE sessions registradas', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/sessions' && r.method === 'GET'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/sessions/:id' && r.method === 'DELETE'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/sessions/revoke-others' && r.method === 'POST'))
  })

  test('GET/DELETE apps registradas', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/apps' && r.method === 'GET'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/apps/:clientId' && r.method === 'DELETE'))
  })

  test('GET /account/api/mfa registrada', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/mfa' && r.method === 'GET'))
  })

  test('GET/DELETE passkeys registradas', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/passkeys' && r.method === 'GET'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/passkeys/:id' && r.method === 'DELETE'))
  })

  test('GET/POST/DELETE tokens registradas', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/tokens' && r.method === 'GET'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/tokens' && r.method === 'POST'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/tokens/:id' && r.method === 'DELETE'))
  })

  test('GET orgs + invitations + :id registradas (invitations ANTES de :id)', ({ assert }) => {
    const router = fakeRouter()
    registerAuthHost(router, { mountPath: '/oidc' })
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/orgs' && r.method === 'GET'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/orgs/invitations' && r.method === 'GET'))
    assert.isTrue(router.routes.some((r: any) => r.pattern === '/account/api/orgs/:id' && r.method === 'GET'))
    ...
    assert.isBelow(idxInvitations, idxOrgId, '/account/api/orgs/invitations deve ser registrada antes de /account/api/orgs/:id')
  })
  ...
  test('email-change/cancel registrada ANTES de /email-change (anti-shadowing)', ({ assert }) => {
    ...
    assert.isBelow(idxCancel, idxEmailChange, '/account/api/email-change/cancel deve vir antes de /account/api/email-change')
  })
})
