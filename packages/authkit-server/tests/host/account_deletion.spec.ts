...
async function seedArtifacts(db: any, accountId: string, grantId: string) {
  ...
  await session.upsert(`sess-${accountId}`, { accountId, loginTs: 1700000000, amr: ['pwd'] } as any, 3600)
...
test.group('AccountDeletionService (cascade LGPD/GDPR)', (group) => {
  ...
  test('cascade completo: cada artefato some, audit anonimizado mas preservado', async ({
    ...
    const acc = await store.create({ email: 'victim@acme.test', password: 'pass123456', fullName: 'V' })
    await store.linkProviderIdentity({ accountId: acc.id, provider: 'google', providerUserId: 'g-1', email: 'v@g.com' })
    ...
    await cfg.audit!.record({ type: 'login.success', accountId: acc.id, email: acc.email, ip: '1.2.3.4' })
    await cfg.audit!.record({ type: 'pat.issued', accountId: acc.id, email: acc.email, ip: '1.2.3.4' })
  ...
  test('store sem deleteAccount: canDelete=false e delete() é no-op', async ({ assert, cleanup }) => {
    const port = 9872
    ...
    delete (stripped as any).deleteAccount
...
test.group('AccountExportService (portabilidade LGPD/GDPR)', (group) => {
  ...
  test('export reúne dados e NÃO vaza segredos (token/hash/key material)', async ({
    ...
    const acc = await store.create({ email: 'me@acme.test', password: 'pass123456', fullName: 'Me' })
    await store.linkProviderIdentity({ accountId: acc.id, provider: 'github', providerUserId: 'h-9', email: 'me@gh.com' })
    ...
    await cfg.audit!.record({ type: 'login.success', accountId: acc.id, email: acc.email, ip: '8.8.8.8' })
