...
import type { AuditEvent, AuditPage, AuditSink, StoredAuditEvent } from '../../src/audit/audit_sink.js'
...
test.group('isAccountExpired', () => {
  test('returns false when setting disabled', async ({ assert }) => {
    ...
    const audit = makeAuditWithList([
      makeStoredEvent('login.success', ACCOUNT_ID, new Date(Date.now() - 400 * 24 * 60 * 60 * 1000))
    ])
  ...
  test('returns false when account never logged in (no login.success events)', async ({ assert }) => {
    const settings = makeSettings({ enabled: true, inactiveDays: 30, warnDays: 7 })
  ...
  test('returns true when last login is beyond inactiveDays threshold', async ({ assert }) => {
    ...
    const audit = makeAuditWithList([
      makeStoredEvent('login.success', ACCOUNT_ID, lastLogin)
    ])
    const result = await isAccountExpired(audit, ACCOUNT_ID, settings)
  ...
  test('reset de senha reativa: após reset.consumed, novo login.success reseta o clock', async ({ assert }) => {
...
test.group('expire-scan mechanics', () => {
  test('dry-run scan: detecta contas expiradas e a expirar, sem auditar nem enviar e-mail', async ({ assert }) => {
    ...
    const auditEvents: StoredAuditEvent[] = [
      ...
      makeStoredEvent('login.success', ACCOUNT_EXPIRED.id, new Date(Date.now() - 70 * 24 * 60 * 60 * 1000)),
      // acc-warn: último login há 55 dias (entre 50 e 60 → a expirar em 5d)
      makeStoredEvent('login.success', ACCOUNT_WARN.id, new Date(Date.now() - 55 * 24 * 60 * 60 * 1000)),
      // acc-active: último login há 10 dias
      makeStoredEvent('login.success', ACCOUNT_ACTIVE.id, new Date(Date.now() - 10 * 24 * 60 * 60 * 1000)),
    ]

    const auditRecorded: AuditEvent[] = []
    const audit: AuditSink = {
      async record(e) { auditRecorded.push(e) },
      async list(params): Promise<AuditPage> {
    ...
    const fakeApp = {
      ...
      container: { make: async () => { throw new Error('no-db') } },
    } as any
    ...
    const { resolveEffectiveAccountExpiration: resolve } = await import('../../src/host/runtime_toggles.js')
    ...
    const warnCutoffMs = nowMs - (expiration.inactiveDays - expiration.warnDays) * 24 * 60 * 60 * 1000
    ...
    for (const acc of accounts) {
      ...
      if (lastMs < expiryCutoffMs) {
        ...
        const daysUntilExpiry = Math.ceil((lastMs + expiration.inactiveDays * 24 * 60 * 60 * 1000 - nowMs) / (24 * 60 * 60 * 1000))
  ...
  test('warn dedup: não re-avisa conta que já foi avisada dentro da janela warnDays', async ({ assert }) => {
    ...
    const res = await audit.list!({ type: 'account.expiration_warned', subject: ACCOUNT_WARN, page: 1, limit: 1 })
