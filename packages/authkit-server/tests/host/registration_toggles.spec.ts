...
test.group('AuthRegistrationController — registration disabled', () => {
  test('GET signup shows registrationDisabled=true when setting { enabled: false }', async ({
    assert,
  }) => {
  ...
  test('GET signup shows form normally when registration enabled (no setting)', async ({
    assert,
  }) => {
    const db = noTableDb();
    const service = buildService({ db });
  ...
  test('GET signup shows form when config static enabled=false (setting absent → config default)', async ({
    assert,
  }) => {
  ...
  test('GET signup shows form when setting overrides config to enabled=true', async ({
    assert,
  }) => {
