...
function fakeRuntimeSettings(value: Record<string, unknown> | null) {
  return {
    async isTablePresent() { return value !== null },
    async getSetting(key: string) { return value && (value[key] ?? null) },
    async setSetting() {},
    async deleteSetting() {},
  } as any
}
...
test.group('resolveEffectiveSecurityNotifications', () => {
  ...
  test('fail-safe quando getSetting lança', async ({ assert }) => {
    const failSettings = {
      async isTablePresent() { return true },
      async getSetting() { throw new Error('db down') },
    } as any
...
test.group('resolveEffectiveEmailChange', () => {
  ...
  test('respeita requirePassword: false', async ({ assert }) => {
    const settings = fakeRuntimeSettings({ email_change: { enabled: true, requirePassword: false } })
    const result = await resolveEffectiveEmailChange(settings)
...
test.group('dispatchSecurityNotice', (group) => {
  ...
  function mailStub(sent: any[]) {
    return {
      send: async (cb: any) => {
        const msg: any = {
          _from: undefined, _to: undefined, _subject: undefined, _html: undefined, _text: undefined,
          from(v: any) { this._from = v; return this },
          to(v: any) { this._to = v; return this },
          subject(v: any) { this._subject = v; return this },
          html(v: any) { this._html = v; return this },
  ...
  test('não envia quando enabled: false na setting', async ({ assert }) => {
    ...
    await dispatchSecurityNotice(
      ctx,
      { account: { id: 'u1', email: 'user@acme.example.com' }, kind: 'password_changed', timestamp: '2026-06-06T00:00:00Z' },
      undefined,
      undefined
    )
  ...
  test('não envia para kind não incluído nos kinds da setting', async ({ assert }) => {
    ...
    const db = fakeDbWithSettings({ security_notifications: { enabled: true, kinds: ['mfa_enabled'] } })
    const logger = fakeLogger()
    const ctx = fakeCtx(logger, { containerDb: db })
  ...
  test('usa hook onSecurityNotice do config quando fornecido (substitui default)', async ({ assert }) => {
    const hookCalls: any[] = []
    const sent: any[] = []
    __setMailLoaderForTests(() => Promise.resolve(mailStub(sent)))
    ...
    const mailHooks = {
      onSecurityNotice: async (data: any) => { hookCalls.push(data) },
    }
  ...
  test('fail-safe quando hook lança — não propaga o erro', async ({ assert }) => {
    ...
    const mailHooks = {
      onSecurityNotice: async () => { throw new Error('hook explodiu') },
    }
    ...
    await assert.doesNotReject(() =>
      dispatchSecurityNotice(
        ctx,
        { account: { id: 'u1', email: 'user@acme.example.com' }, kind: 'mfa_enabled', timestamp: '2026-06-06T00:00:00Z' },
        mailHooks,
        undefined
      )
    )
  ...
  test('audita security_notice.sent quando enviado', async ({ assert }) => {
    __setMailLoaderForTests(() => Promise.resolve({
      send: async (cb: any) => {
        const msg: any = { from() { return this }, to() { return this }, subject() { return this }, html() { return this }, text() { return this } }
        await cb(msg)
      },
    }))
    ...
    const audit = { record: async (e: AuditEvent) => { auditEvents.push(e) } }

    await dispatchSecurityNotice(
      ctx,
      { account: { id: 'u1', email: 'user@acme.example.com' }, kind: 'mfa_disabled', timestamp: '2026-06-06T00:00:00Z' },
      undefined,
      audit as any
    )
  ...
  test('todos os kinds são aceitos por default (tabela ausente = defaults)', async ({ assert }) => {
    ...
    const mailHooks = {
      onSecurityNotice: async (data: any) => { hookCalls.push(data.kind) },
    }

    const kinds: SecurityNotificationKind[] = [
      'password_changed', 'mfa_enabled', 'mfa_disabled',
      'passkey_added', 'passkey_removed', 'email_changed',
    ]
...
test.group('email_change mailer functions', (group) => {
  ...
  function mailStub(sent: any[]) {
    return {
      send: async (cb: any) => {
        const msg: any = {
          _to: undefined, _subject: undefined, _html: undefined, _text: undefined,
          from() { return this }, to(v: any) { this._to = v; return this },
          subject(v: any) { this._subject = v; return this },
          html(v: any) { this._html = v; return this }, text(v: any) { this._text = v; return this },
        }
        await cb(msg)
        sent.push(msg)
      },
  ...
  test('sendEmailChangeNoticeEmail envia para o e-mail atual (OLD)', async ({ assert }) => {
    ...
    await sendEmailChangeNoticeEmail(ctx, { email: 'old@acme.example.com', newEmail: 'new@acme.example.com' })

    assert.equal(sent.length, 1)
    assert.equal(sent[0]._to, 'old@acme.example.com')
    assert.ok(sent[0]._subject?.includes('change') || sent[0]._subject?.includes('troca'),
      'assunto deve mencionar troca')
    assert.ok(sent[0]._html?.includes('new@acme.example.com'),
  ...
  test('sendEmailChangedCompletedEmail envia para o e-mail ANTIGO com OLD+NEW', async ({ assert }) => {
    const sent: any[] = []
    __setMailLoaderForTests(() => Promise.resolve(mailStub(sent)))
    ...
    await sendEmailChangedCompletedEmail(ctx, { oldEmail: 'old@acme.example.com', newEmail: 'new@acme.example.com' })

    assert.equal(sent.length, 1)
    assert.equal(sent[0]._to, 'old@acme.example.com')
    assert.ok(sent[0]._html?.includes('old@acme.example.com'), 'corpo deve mencionar o e-mail antigo')
    assert.ok(sent[0]._html?.includes('new@acme.example.com'), 'corpo deve mencionar o e-mail novo')
  })
  ...
  test('sendEmailChangedCompletedEmail fail-safe: nunca lança', async ({ assert }) => {
    ...
    await assert.doesNotReject(() =>
      sendEmailChangedCompletedEmail(ctx, { oldEmail: 'old@acme.example.com', newEmail: 'new@acme.example.com' })
    )
  ...
  test('sendSecurityNoticeEmail envia e-mail com o kind correto no assunto', async ({ assert }) => {
    ...
    assert.ok(sent[0]._subject?.toLowerCase().includes('password') || sent[0]._subject?.toLowerCase().includes('senha'),
