...
test.group('sudoMethods.password', () => {
  ...
  test('disponível quando o store não expõe __getRawRow (fallback conservador)', async ({
    assert,
  }) => {
  ...
  test('disponível quando o store expõe __getRawRow e lança (fallback conservador)', async ({
    assert,
  }) => {
    const c = ctxWith({
      accountStore: {
