...
function fakeProvider() {
  ...
  class Grant {
    ...
    clientId: string;
    scopes: string[] = [];
    constructor(o: { accountId: string; clientId: string }) {
  ...
  return {
    ...
    async interactionDetails() {
      return {
        uid: 'i1',
        prompt: { name: 'consent' },
        params: { client_id: 'app1', scope: 'openid email' },
        session: { accountId: 'u1' },
      };
    },
    async interactionFinished(_req: any, _res: any, result: any) {
      calls.finished = result;
      return 'redirected';
    },
  } as any;
}
...
test.group('createInteractionActions', () => {
  ...
  test('login sem verifyCredentials configurado lança erro claro', async ({ assert }) => {
    ...
    await assert.rejects(
      () => actions.login(ctx, { email: 'a', password: 'b' }),
      /verifyCredentials/,
    );
  });
  ...
  test('completeLogin chama interactionFinished({login}) com o accountId dado', async ({
    assert,
  }) => {
    const provider = fakeProvider();
    const actions = createInteractionActions(provider, {});
