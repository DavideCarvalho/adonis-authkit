...
function fakeVault() {
  ...
  const fetchImpl = async (url: string, init?: any) => {
    ...
    if (method === 'GET' && isData) {
      ...
      return { ok: true, status: 200, json: async () => ({ data: { data: { value: e.value }, metadata: { version: e.version } } }) }
    }
    ...
    if (method === 'GET' && !isData) {
      const e = store.get(path)
      if (!e) return { ok: false, status: 404, json: async () => ({}) }
      return { ok: true, status: 200, json: async () => ({ data: { current_version: e.version } }) }
    }
...
test.group('HashicorpVaultKeystoreVault', () => {
  ...
  test('erro HTTP não-404 no read → lança (chave crítica)', async ({ assert }) => {
    ...
    const v = new HashicorpVaultKeystoreVault({ endpoint: 'http://vault:8200', path: 'p', token: 't' }, fetchImpl as any)
