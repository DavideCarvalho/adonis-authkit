...
test.group('OIDC capabilities (e2e)', (group) => {
  ...
  group.setup(async () => {
    ...
    const cfg = await configProvider.resolve(
      ...
      defineConfig({
        ...
        clients: [
          { clientId: 'app1', clientSecret: 's', redirectUris: [`${ISSUER}/cb`], grants: ['authorization_code', 'refresh_token'] },
        ],
  ...
  test('authorize SEM PKCE é rejeitado (PKCE obrigatório)', async ({ assert }) => {
    ...
    const location = res.headers.get('location') ?? ''
    const rejected = res.status === 400 || location.includes('error=') || !location.includes('/interaction/')
