...
function buildCfg(extra: Partial<any> = {}) {
  ...
  return {
    cfg: {
      ...
      audit: {
        events: [] as any[],
        record: async (e: any) => { (cfg as any).audit.events.push(e) },
      },
      mail: {
        onOrgInvitation: async (data: any) => { mailInvitations.push(data) },
      },
      ...extra,
    } as any,
    mailInvitations,
...
test.group('AdminOrgsService', (group) => {
  ...
  test('createOrg + listOrgs retorna a org criada', async ({ assert }) => {
    ...
    const result = await svc.createOrg({ name: 'Acme', slug: 'acme', ownerAccountId: 'owner-1' }, actor)
  ...
  test('createOrg retorna slug_taken quando slug duplicado', async ({ assert }) => {
    ...
    const actor = { actorId: null, ip: null, source: 'admin-api' as const }
    await svc.createOrg({ name: 'Acme', slug: 'acme', ownerAccountId: 'owner-1' }, actor)
    const result = await svc.createOrg({ name: 'Acme2', slug: 'acme', ownerAccountId: 'owner-1' }, actor)
  ...
  test('getOrg retorna detalhe com membros e convites', async ({ assert }) => {
    const svc = new AdminOrgsService(cfg)
    const actor = { actorId: null, ip: null, source: 'admin-api' as const }
    const created = await svc.createOrg({ name: 'Acme', slug: 'acme', ownerAccountId: 'owner-1' }, actor) as any
  ...
  test('createInvitation dispara mail hook', async ({ assert }) => {
    ...
    const result = await svc.createInvitation(org.id, { email: 'invited@x.com', role: 'member' }, actor, 'http://localhost')
...
test.group('ApiOrgsController', (group) => {
  ...
  test('PATCH /organizations/:id atualiza nome', async ({ assert }) => {
    const org = await seedOrg()
    const ctrl = new ApiOrgsController()
    const res: any = await ctrl.update(fakeCtx({ params: { id: org.id }, inputs: { name: 'New Name' } }).ctx)
  ...
  test('DELETE /organizations/:id/members/:accountId remove membro', async ({ assert }) => {
    const org = await seedOrg()
    await new AdminOrgsService(cfg).addMember(org.id, { accountId: 'user-2', role: 'member' }, { actorId: null, ip: null })
    const ctrl = new ApiOrgsController()
    const res: any = await ctrl.removeMember(fakeCtx({ params: { id: org.id, accountId: 'user-2' } }).ctx)
  ...
  test('PATCH /organizations/:id/members/:accountId troca role', async ({ assert }) => {
    ...
    await new AdminOrgsService(cfg).addMember(org.id, { accountId: 'user-2', role: 'member' }, { actorId: null, ip: null })
    const ctrl = new ApiOrgsController()
    const res: any = await ctrl.updateMemberRole(fakeCtx({ params: { id: org.id, accountId: 'user-2' }, inputs: { role: 'admin' } }).ctx)
    assert.equal(res.role, 'admin')
  })

  test('POST /organizations/:id/invitations cria convite (201) + dispara mail', async ({ assert }) => {
    const org = await seedOrg()
    const ctrl = new ApiOrgsController()
    const c = fakeCtx({ params: { id: org.id }, inputs: { email: 'invited@x.com', role: 'member' } })
  ...
  test('DELETE /organizations/:id/invitations/:invitationId revoga convite', async ({ assert }) => {
    ...
    const ctrl = new ApiOrgsController()
    const res: any = await ctrl.revokeInvitation(fakeCtx({ params: { id: org.id, invitationId: inv.invitation.id } }).ctx)
