...
test.group('PasswordManager.verify com pepper', () => {
  ...
  test('legacy sem pepper: confere com sentinel "" → rehash=true (re-hash com pepper)', async ({ assert }) => {
    const pm = new PasswordManager({ pepper: 'pepper1' })
  ...
  test('rotação: pepper corrente não gera rehash (só parâmetros desatualizados)', async ({ assert }) => {
    const pm = new PasswordManager({ pepper: ['newPepper', 'oldPepper'] })
