...
test.group('KeyRotationScheduler', () => {
  ...
  test('re-checa idade DENTRO do lock (evita dupla rotação)', async ({ assert }) => {
    ...
    const { scheduler, calls } = sched({
      ageDays: async () => age,
      withLock: async (fn: () => Promise<void>) => { age = 0; await fn() }, // outra instância rotacionou
    })
    await scheduler.tick()
  ...
  test('passa keep da política ao rotateKeys', async ({ assert }) => {
    let keepUsed = -1
    const { scheduler } = sched({ policy: async () => ({ enabled: true, maxAgeDays: 90, keep: 4 }), rotateKeys: async (k: number) => { keepUsed = k } })
    await scheduler.tick()
